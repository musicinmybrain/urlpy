...
@unittest.skip('six not available')
def test_string_url():
    parsed = url.StringURL.parse('http://user@example.com:80/path;params?query#fragment')
    properties = [
        'scheme', 'host', 'params', 'query', 'fragment', 'userinfo', 'pld', 'tld'
    ]
    for prop in properties:
        assert isinstance(getattr(parsed, prop), six.binary_type)

@unittest.skip('six not available')
def test_unicode_url():
    parsed = url.UnicodeURL.parse('http://user@example.com:80/path;params?query#fragment')
    properties = [
        'scheme', 'host', 'params', 'query', 'fragment', 'userinfo', 'pld', 'tld'
    ]
    for prop in properties:
